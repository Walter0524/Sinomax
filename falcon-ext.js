//定制块：恢复全局变量
function restoreGlobalValue(tc, params) {
...
//定制块：按巷道找空库位
function findEmptyBinByChannel(tc, params) {
...
//定制块：尝试重置巷道出入状态
function tryToResetChannelIoState(tc, params) {
    const falconTaskId = params["falconTaskId"];
...
//定制块：尝试清除巷道物料信息
function tryToResetChannelMaterial(tc, params) {
    const channelId = params["channelId"];
...
function findEmptyContainerByDistrict(tc, params) {
